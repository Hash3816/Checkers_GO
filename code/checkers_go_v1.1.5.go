...
func (game *Game) Update() error {
	...
	if game.Menu.Active {
		if inpututil.IsMouseButtonJustPressed(ebiten.MouseButtonLeft) {
			...
			} else if pos.In(game.Menu.GameWithAI) {
				*game = *initGame()
				game.Menu.Active = false
				game.InGame = true
				game.AI = true
	...
	if game.AI && game.Now_player == "black" {
		game.moveAI()
		return nil
	}
...
func (game *Game) Draw(screen *ebiten.Image) {
	if game.Menu.Active {
		...
		ebitenutil.DrawRect(screen, float64(startBtn.Min.X), float64(startBtn.Min.Y),
			float64(startBtn.Dx()), float64(startBtn.Dy()), color.RGBA{34, 197, 94, 255})
		ebitenutil.DrawRect(screen, float64(AI_Btn.Min.X), float64(AI_Btn.Min.Y),
			float64(AI_Btn.Dx()), float64(AI_Btn.Dy()), color.RGBA{10, 26, 42, 255})
		...
		ebitenutil.DrawRect(screen, float64(exitBtn.Min.X), float64(exitBtn.Min.Y),
			float64(exitBtn.Dx()), float64(exitBtn.Dy()), color.RGBA{107, 114, 128, 255})

		// Текст на кнопках
		text.Draw(screen, "Начать игру", russianFont, startBtn.Min.X+48, startBtn.Min.Y+30, color.White)
		text.Draw(screen, "Играть с ИИ", russianFont, AI_Btn.Min.X+48, AI_Btn.Min.Y+30, color.RGBA64{0, 52942, 53713, 65535})
		...
		// Заголовок
		text.Draw(screen, "РУССКИЕ ШАШКИ", SpecialFont, board_Size/2-107, board_Size/2-67, color.White)
...
func initGame() *Game {
	game := &Game{
		...
		Menu: Menu{
			...
			GameWithAI:     image.Rect(board_Size/2-100, board_Size/2+20, board_Size/2+100, board_Size/2+70),
			ContinueButton: image.Rect(board_Size/2-100, board_Size/2+90, board_Size/2+100, board_Size/2+140),
			RulesButton:    image.Rect(board_Size/2-100, board_Size/2+160, board_Size/2+100, board_Size/2+210),
			ExitButton:     image.Rect(board_Size/2-100, board_Size/2+230, board_Size/2+100, board_Size/2+280),
			Active:         true,
		},
		RulesScreen: RulesScreen{
...
func init() {
	...
	fontData, err := ioutil.ReadFile("fonts/CCGothChic Black Regular.ttf")
	if err != nil {
		log.Fatal(err)
	}
	...
	SpecialFont, err = opentype.NewFace(Font, &opentype.FaceOptions{
		Size: 24,
		DPI:  72,
	})
}
func drawBear(screen *ebiten.Image) {
	...
	ebitenutil.DrawCircle(screen, float64(board_Size/2), float64(board_Size/1.5), 210, color.RGBA{139, 69, 19, 255})

	// Голова — меньший круг сверху
	ebitenutil.DrawCircle(screen, float64(board_Size/2), float64(board_Size/1-400), 115, color.RGBA{139, 69, 19, 255})
